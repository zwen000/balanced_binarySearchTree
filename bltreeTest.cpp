...
object_t *deletekey(tree_node_t *tree, key_t delete_key)
{  
...
{ 
	if(tree->height == 0)
	{
		if((a <= tree->key)&&(tree->key <= b))  return 1;	
		else  return 0;  
	}
	else { 
...
////////////////////////////////Final////////////////////////////////////////
int count_keys2(tree_node_t *tree,key_t a, key_t b) 
{ 
	if(a>b) return 0 ; 
	if(tree->height == 0)
	{
		...
		if(tree->key < a)
			return count_keys2(tree->right,a,b);
		else if(tree->key > b)
			return count_keys2(tree->left,a,b); 
		else
			return count_keys2(tree->left,a,b) + count_keys2(tree->right,a,b);
	}
}
 /*
int countAllBiggerleafs(tree_node_t * , key_t  ) ;
int countAllSmallerleafs(tree_node_t * , key_t  );
...
if (tree == NULL)
 ...
 return countAllBiggerleafs(tree->left, a) + countAllSmallerleafs(tree->right, b);
 
}

int countAllBiggerleafs(tree_node_t *tree, key_t x) {
 if (tree == NULL)
 return 0;
 if (tree->key == x && tree->left == NULL && tree->right == NULL)
 return 1;
 
 if (tree->key < x)
 return countAllBiggerleafs(tree->right, x);
 else
 return countAllBiggerleafs(tree->left, x);
}

int countAllSamllerleafs(tree_node_t *tree, key_t x) {
 if (tree == NULL)
 return 0;
 if ( tree->key == x && tree->left == NULL && tree->right == NULL)
 return 1;
 ...
 if (tree->key > x)
 return countAllSmallerleafs(tree->left, x);
 
 else
 return countAllSmallerleafs(tree->right, x);
 }
